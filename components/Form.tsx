...
const Form: FC<Props> = ({ prompt, setPrompt, setApiResponses }) => {
  ...
  return (
    <form
      className='mx-auto grid w-full justify-items-center gap-8 rounded-lg bg-slate-50 px-8 py-6 shadow-md 
      dark:border-slate-50/[.08] dark:bg-slate-800'
      onSubmit={handleSubmit}
    >
      <label className='flex w-full flex-col gap-2'>
        <textarea
          className='animate resize-none rounded-md border border-blue-300 bg-slate-100 p-4 
          focus-visible:shadow-inner focus-visible:shadow-blue-300 focus-visible:outline-2 focus-visible:outline-blue-300 
          disabled:bg-slate-400 dark:bg-slate-900 dark:text-slate-100 disabled:dark:bg-slate-400'
          name='prompt'
          id='prompt'
          rows={10}
      ...
      <button
        className='animate relative w-fit rounded-md bg-blue-800 px-8 py-2 font-semibold text-white  
        hover:scale-105 disabled:bg-slate-400 dark:bg-blue-600 disabled:dark:bg-slate-600'
        disabled={isSubmitting}
      >
        {isSubmitting && (
